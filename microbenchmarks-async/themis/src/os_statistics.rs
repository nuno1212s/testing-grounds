...
pub fn start_statistics_thread(node_id: u64) {
...
fn take_ram_measurement(node_id: u64) {
    let memory_stats = procinfo::pid::statm_self().unwrap();

    let timestamp = Utc::now().timestamp_millis();

    println!("NodeId({:?}) // {:?} // RAM Usage {}", node_id, timestamp,
             memory_stats.data
    );
}

fn take_cpu_measurement(node_id: u64) {
    ...
    for (cpu_id, usage) in cpu_res {
        println!("NodeId({:?}) // {:?} // CPU {} utilization: {}", node_id, timestamp, cpu_id, usage);
    }

    println!("NodeId({:?}) // {:?} // Network TX Usage {} Network RX Usage {}", node_id, timestamp, tx_speed, rx_speed);
}