...
public class AsyncLatencyClient {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int concurrentRqs = Integer.parseInt(args[3]);
        int requestSize = Integer.parseInt(args[4]);
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            clients[i] = new AsyncLatencyClient.Client(initId + i, numberOfOps, concurrentRqs, requestSize, interval, readOnly, verbose, s);
        }
    ...
    static class Client extends Thread {
        ...
        public Client(int id, int numberOfOps, int concurrentRqs, int requestSize, int interval, boolean readOnly, boolean verbose, int sign) {

            this.id = id;
            this.serviceProxy = new AsynchServiceProxy(id);

            this.numberOfOps = numberOfOps;
            this.concurrentRqs = concurrentRqs;
                    ...
                    this.serviceProxy.invokeAsynchRequest(this.request, new ReplyListener() {
                        ...
                        public void replyReceived(RequestContext context, TOMMessage reply) {
                            ...
                            if (replies >= q) {
                                ...
                                if (finalI > (numberOfOps / 2)) st.store(System.nanoTime() - last_send_instant);
                            }
                        }
                    }, this.reqType);

                    if (this.interval > 0 || this.rampup > 0) {
                        Thread.sleep(Math.max(rand.nextInt(this.interval + 1) + 1, this.rampup));
