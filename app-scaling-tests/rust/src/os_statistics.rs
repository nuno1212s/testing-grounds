...
fn take_ram_measurement(node_id: NodeId) {
    ...
    println!(
        "{:?} // {:?} // RAM Usage {}",
        node_id, timestamp, memory_stats.data
    );
}

fn take_cpu_measurement(node_id: NodeId) {
    let result =
        mprober_lib::cpu::get_all_cpu_utilization_in_percentage(false, Duration::from_millis(100))
            .unwrap();
    ...
    let network_speed =
        mprober_lib::network::get_networks_with_speed(Duration::from_millis(100)).unwrap();
    ...
    for (cpu_id, usage) in cpu_res {
        println!(
            "{:?} // {:?} // CPU {} utilization: {}",
            node_id, timestamp, cpu_id, usage
        );
    }

    println!(
        "{:?} // {:?} // Network TX Usage {} Network RX Usage {}",
        node_id, timestamp, tx_speed, rx_speed
    );
}
