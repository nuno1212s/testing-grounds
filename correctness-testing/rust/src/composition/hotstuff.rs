//! HotIron (HotStuff) replica composition. Mirrors `febft` but swaps in the HotIron
//! ordering protocol + its threshold-crypto `QuorumInfo`. Built on the current-Atlas
//! executor pattern (the microbenchmark-hotstuff reference uses a stale `Exec` alias).
#![allow(dead_code)]
...
pub type OrderProtocolMessage = HotIronOxSer<SMRReq<AppData>>;
...
pub type OrderProtocol =
    HotIron<SMRReq<AppData>, ProtocolNetwork, QuorumInfo, RequestPreProcessor<SMRReq<AppData>>>;
