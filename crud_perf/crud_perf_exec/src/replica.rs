...
use crate::common::{MonConfig, ReplicaConf, SMRReplica, generate_log};
...
pub fn init_replica_config(
    ...
    let db_path = match db_path {
        Ok(db) => db,
        Err(_) => return Err(anyhow!("Failed to parse persistent log folder")),
    };
...
pub(super) fn run_replica() {
    ...
    let (network_cfg, _pool_config) = get_network_configurations(node_id).unwrap();
